...
def checkSimilarity(window, path1, path2):
    ...
    if(result <= THRESHOLD):
        messagebox.showerror("Failure: Handwriting Do Not Match",
                             "Handwriting are "+str(result)+f" % similar!!")
        pass
    else:
        messagebox.showinfo("Success: Handwriting Match",
                            "Handwriting are "+str(result)+f" % similar!!")
    return True


root = tk.Tk()
root.title("Handwriting OCR Verification")
root.geometry("500x700")  # 300x200
uname_label = tk.Label(root, text="Compare Two Handwritings:", font=10)
uname_label.place(x=90, y=50)

img1_message = tk.Label(root, text="Handwriting 1", font=10)
...
img2_message = tk.Label(root, text="Handwriting 2", font=10)
